...
- [Памятка по современному JavaScript](#Памятка-по-современному-JavaScript)
  ...
  * [Понятия](#Понятия)
    ...
    + [Значение параметра функции по умолчанию](#Значение-параметра-функции-по-умолчанию)
      - [Внешние ресурсы](#Внешние-ресурсы-1)
    ...
    + [Методы массивов - map / filter / reduce](#Методы-массивов---map--filter--reduce)
      ...
      - [Внешние ресурсы](#Внешние-ресурсы-2)
    ...
    + [Импорт / экспорт](#Импорт--экспорт)
      ...
      - [Внешние ресурсы](#Внешние-ресурсы-8)
    + [*this* в JavaScript](#-this-в-javascript)
      - [Внешние ресурсы](#Внешние-ресурсы-9)
    + [Класс](#Класс)
      - [Примеры](#Примеры)
      - [Внешние ресурсы](#Внешние-ресурсы-10)
    + [Async Await](#async-await)
      - [Пример кода](#Пример-кода-6)
      - [Объяснение с примером кода](#Объяснение-с-примером-кода-2)
      - [Внешние ресурсы](#Внешние-ресурсы-11)
    ...
    + [Статические методы](#Статические-методы)
      ...
      - [Внешние ресурсы](#Внешние-ресурсы-12)
  * [Глоссарий](#Глоссарий)
...
Это сложное понятие с множеством тонкостей, так что я крайне рекомендую вам тщательно изучить приведенные ниже внешние ресурсы. Я покажу вам, как сам лично определяю, чему равно ```this```. Этому меня научила [вот эта статья Yehuda Katz](http://yehudakatz.com/2011/08/11/understanding-javascript-function-invocation-and-this/).
```js
function myFunc() {
  ...
}
...
// ключевое слово async позволяет использование await в функции и означает, что функция возвращает промис
  try {
    // Вот так обрабатываются ошибки с помощью async / await
...
т.к. этот код не находится внутри async-функции. */
```
...
// превращение в промис + разрешено использование ключевого слова await
  try {
  // Мы обрабатываем ошибки в асинхронной функции с помощью try / catch
    const response = await fetch(`https://api.github.com/users/${username}`);
    // Выполнение останавливается здесь, пока не закончится выполнение промиса.
    const user = response.json();
    return user;
    // эквивалент разрешения промиса getGithubUser со значением user.
  } catch (err) {
    throw new Error(err);
    // эквивалент отклонения промиса getGithubUser со значением err.
  }
}
getGithubUser('mbeaudru')
.then(user => console.log(user))
