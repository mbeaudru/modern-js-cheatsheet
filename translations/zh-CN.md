...
ES6 JS 的更新版本已经介绍了*箭头函数*， 是声明和使用函数的另一种方式。以下是箭头函数带来的一些好处：
...
在箭头函数中， *this*的值就等于它所处的封闭的可执行上下文的*this*。简单来说，就是在箭头函数中，当你调用一个位于函数内部的函数时，你不需要使用“that = this” 这样的声明语句。
...
箭头函数从很多方面都比传统的函数简洁了很多。案例如下：

- 隐式返回 VS 显式返回

**显式返回** 指的是函数体内使用了return 关键字
...
传统函数总是伴随着显式返回。使用箭头函数，你可以使用*隐式返回*，即不需要在函数体内使用return关键字来返回一个值。

隐式返回需要将返回值的代码写在一条语句中。
...
为实现隐式返回，我们只需要 **移除花括号** 和 **return** 关键字。之所以被称为*隐式*返回，是因为*return*关键字不存在的情况下，函数却可以正确返回 ```x * 2```。

> **注意:** 如果你的函数不是返回一个单值（伴随着*副作用*），那么既不可以使用显式返回也不可以使用隐式返回。

除此之外， 由于对象的花括号可能会与表示块作用域的花括号起冲突，如果你想隐式返回一个*object* 则必须使用圆括号对其修饰， 
...
要理解箭头函数中this的微妙，你必须首先了解在JavaScript中[this](#this_def) 的行为。

在箭头函数中， *this*的值就等于它所处的封闭的可执行上下文的*this*。这句话的意思就是箭头函数不是创建一个新的*this*， 而是从其所处的上下文环境获取。

使用非箭头函数，如果你想通过*this*使用一个位于另一个函数内部的函数的变量，你需要使用*that = this*或者*self = this*。
...
function myFunc() {
  ...
  setTimeout(
    function() { // 在函数的内部创建一个新的
      ...
      console.log(this.myVar) // 未定义 -- 请参照上面的函数定义
    },
    0
  );
}
